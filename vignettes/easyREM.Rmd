...
In order to create REM objects, centerline of river channels are needed. Rivers data can be retrieved in many ways, the following process exemplifies how do it by:
...
Another way to retrieve a rivers shapefile is through the [HydroSheeds](https://www.hydrosheds.org) project. To facilitate the process, *easyREM* package contains a function that downloads it to your current folder as a .shp file. The function requires a region that is sourced from the HydroSheeds region mapping, which includes Global and regions such as Africa, South America, Asia, Australia/Oceania, Europe, and North America.
...
To download all the respectivelly shapefiles from HydroSheeds project, the another function in *easyREM* function deals with it.
...
Extracting elevation values is a strategy for populating points that matches the river shapefile with elevation from the DEM raster. It is an crucial step to develop the REM model.
